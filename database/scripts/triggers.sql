...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM inserted i
        WHERE ISNULL(i.userCreation, 1) IN (SELECT idUser FROM Users);
    END TRY
    BEGIN CATCH
        -- Registrar error sin causar rollback
        PRINT 'Error en trigger Customers_Insert: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE ISNULL(i.userModification, 1) IN (SELECT idUser FROM Users);
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Customers_Update: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        AND ISNULL(i.userModification, 1) IN (SELECT idUser FROM Users);
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Customers_Delete: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Products_Insert: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Products_Update: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Orders_Insert: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Orders_Update: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    -- Usar TRY...CATCH para evitar que errores en History causen rollback de la operación principal
    BEGIN TRY
        -- Insertar auditoría por cada detalle, obteniendo el usuario desde su propia orden
        INSERT INTO History (affectedTable, actionType, recordId, description, idUser)
        SELECT 
            'OrderDetails',
            'INSERT',
            ...
            ISNULL(o.userCreation, 1) -- Obtener usuario de la orden asociada
        FROM inserted i
        INNER JOIN Orders o ON i.idOrder = o.idOrder
        WHERE ISNULL(o.userCreation, 1) IN (SELECT idUser FROM Users);
    END TRY
    BEGIN CATCH
        -- Registrar error sin causar rollback de la inserción principal
        -- Esto previene que fallos de auditoría causen gaps en IDENTITY
        PRINT 'Error en trigger OrderDetails_Insert: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM inserted i
        WHERE i.idUser IN (SELECT idUser FROM Users);
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Users_Insert: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO
...
BEGIN
    ...
    END TRY
    BEGIN CATCH
        PRINT 'Error en trigger Users_Update: ' + ERROR_MESSAGE();
    END CATCH;
END;
GO